...
class PointOfSales(models.Model):
    """
    An existing AFIP point of sale.

    Points of sales can only to be created via AFIP's website, so you should
    never create instances of this class. You should use
    :meth:`~.TaxPayer.fetch_points_of_sales` to fetch all the existing ones
    programatically.

    Locally deleting or altering these models will not affect upstream point
    of sales; there is no upstream API to alter a POS.
    """
    ...
    blocked = models.BooleanField(
        _("blocked"),
        help_text=(
            "Indicates that this POS has been blocked by AFIP. Note that this "
...
class Receipt(models.Model):
    """
    A receipt (e.g.: invoice, credit note, etc.), as sent to AFIP.

    While AFIP only requires that a range of receipts is sent, these are
    modeled individually in order to store additional data for them -- since
    there's a few key details that are not sent.

    When integrating with this library, you'll probably want a ``ForeignKey``
    on your ``Sale`` or ``Payment`` to relate them with their corresponding
    Receipt.
    Such models are obviously not included because it's expected that you
    already have those, or have very custom need for them.

    If the taxpayer has taxes or pays VAT, you need to attach :class:`~.Tax`
    and/or :class:`~.Vat` instances to each Receipt.
    """
    ...
    total_amount = models.DecimalField(
        ...
        decimal_places=2,
        help_text=_(
            "Must be equal to the sum of net_untaxed, exempt_amount, net_taxed, "
    ...
    def formatted_number(self):
        """The number for this receipt in the official format.
    ...
    def is_validated(self):
        ...
        If you need a large list of receipts, and need their validation status,
        then use a ``annotate`` on your QuerySet.
    ...
    def validate(self, ticket=None, raise_=False):
        ...
        :param AuthTicket ticket: Use this ticket. You can generally omit this
            field and django_afip will do the ticket-handling for you.
